...
# API Endpoint to upload images and generate camouflage
@app.route('/generate-camouflage', methods=['POST'])
def generate_camouflage():
    """
    API endpoint to upload multiple images and generate adaptive camouflage pattern.
    
    Accepts images as multipart form data.
    """
...
# Send the image file as a response
    return send_file(img_io, mimetype='image/png', as_attachment=True, download_name=f"adaptive_camouflage_{timestamp}.png")

    # # Save the image with a unique filename (using timestamp)
    # timestamp = time.strftime("%Y%m%d-%H%M%S")
    # output_filename = f"adaptive_camouflage_{timestamp}.png"
    # output_filepath = os.path.join("generated_camouflages", output_filename)
    
    # camouflage_image.save(output_filepath)

    # return jsonify({"message": "Camouflage generated", "filename": output_filename}), 200

if __name__ == '__main__':
    # Make sure the output directory exists
    os.makedirs("generated_camouflages", exist_ok=True)
